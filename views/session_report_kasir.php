					...
					<table class="t-sesreport table table-hover table-striped text-nowrap table-bordered td-valign-middle" id="tSessionReport">
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td class="d-flex justify-content-between">
										...
										if ($result2->payment_status != '0') : ?>
											...
											<!-- <a href="javascript:;" data-smu="<?php echo $result2->smu; ?> " data-id="<?php echo $result2->payment_id; ?>" data-bs-toggle="modal" data-bs-target="#modalVoid">
											</a> -->
											<button type="button" class="btn btn-outline-danger ms-2" data-id="<?php echo $result2->payment_id; ?>" data-smu="<?php echo $result2->smu; ?>" data-bs-toggle="modal" data-bs-target="#modalVoid">
												void
											</button>
								...
								} elseif ($result2->payment_status == "0") {
									$vsmu = $vsmu + 1;
	...
	<div class="modal-dialog thedialog">
		<div class="modal-content themodal">
			...
			<div class="modal-footer">
				...
				<button type="submit" class="btn btn-warning" name="btn_void" onclick="voidInvoice($(this))">Void</button>
			</div>
			<!-- </form> -->
		</div>
	</div>
...
<script>
	...
	$("#modalVoid").on("show.bs.modal", function(e) {
		...
		console.log(button);
		activeId = button.data('id');
		$("#smux").val(button.data('smu'));
	})

	const voidInvoice = (el) => {
		let payment_id = activeId;
		let keterangan = $("#void").val();
		console.log(payment_id, keterangan);
		if (payment_id != '' && keterangan != '') {
			$.ajax({
				url: 'ajax/invoice.php?action=void_payment',
				type: 'post',
				dataType: 'json',
				data: {
				...
				if (result.status == 200) {
					if (result.sigo == 'success') {
						Swal.fire('Success', 'Void success', 'success')
					} else {
						Swal.fire({
							title: 'Warning',
							text: 'The invoice was successfully canceled but the Sigo data was not successfully cancelled, please try it again in the sigo menu'
						})
					}
					$(document).find(`button[data-id="${payment_id}"]`).parent().html(`void by : ${result.user}`);
				}
			}).catch(error => {
				Swal.fire('Error', 'Void failed', 'error');
			})
		} else {
