						...
						<div class="row">
							<div class="col-sm-3 mb-2">
								...
								<select class="select2 form-select form-select-sm" name="airline" id="airline" data-placeholder="Select Airline">
									<option></option>
									<option value="all" <?= (@$_POST['airline'] && $_POST['airline'] == 'all') ? 'selected' : ''; ?>>All Airline</option>
									<?php
									while ($airline = $d_airline->fetch_object()) {
									?>
										<option value="<?= $airline->airline_name ?>" <?= (@$_POST['airline'] && $_POST['airline'] == $airline->airline_name) ? 'selected' : ''; ?>><?= $airline->airline_name ?></option>
									<?php
									}
									?>
								</select>
						...
						<div class="" id="manifest">
							<div class="row g-0 p-0 m-0">
								<div class="position-relative col-md-2 mt-2 me-2">
									<input class="form-control form-control-sm" type="text" id="tanggalAwal" autocomplete="off" name="date1" value="<?= @$_POST['date1'] ?>">
									<label>Tanggal awal</label>
								</div>
								<div class="position-relative col-md-2 mt-2 me-2">
									<input class="form-control form-control-sm" type="text" id="tanggalAkhir" autocomplete="off" name="date2" value="<?= @$_POST['date1'] ?>">
					...
					<tbody>
						...
						if (@$_POST['airline']) {
							if ($_POST['airline'] == 'all') {
								...
								$takedata = $report->allbydate2($date1, $date2);
							} else {
								// $takedata = $report->allbydate_airline($date1, $date2, "complete", $_POST['airline']);
								$takedata = $report->allbydate_airline2($date1, $date2, $_POST['airline']);
							}
						} else {
							// $takedata = $report->allbydate($date1, $date2, "complete");
							$takedata = $report->allbydate2($date1, $date2);
					...
					<tbody>
						...
						if (@$_POST['airline']) {
							if ($_POST['airline'] == 'all') {
								...
								$takedatavoid = $report->allbydate2($date1, $date2);
							} else {
								$takedatavoid = $report->allbydate_airline($date1, $date2, "void", $_POST['airline']);
								$takedatavoid = $report->allbydate_airline2($date1, $date2, $_POST['airline']);
							}
						} else {
							// $takedatavoid = $report->allbydate($date1, $date2, "void");
							$takedatavoid = $report->allbydate2($date1, $date2);
