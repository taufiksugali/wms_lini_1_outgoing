...
<div class="kontener2 px-5 pt-4 " style="padding-bottom: 10em;">
	<nav style="--bs-breadcrumb-divider: '》';" aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item" aria-current="page">session</h3>
			</li>
			<li class="breadcrumb-item active" aria-current="page">Session report</h3>
			</li>
		</ol>
	</nav>
			...
			<div class="main-form col-sm-10 px-3 ">
				<form class="sesreport" action="" method="post">
					<div>
						<div class="row">
							<div class="col-sm-3 mb-2">
								...
								<select class="select2 form-select form-select-sm" name="airline" id="airline" data-placeholder="Select Airline">
									<option></option>
									<option value="all" <?= @$_POST['session'] && $_POST['airline'] == 'all' ? 'selected' : '' ?>>All Airline</option>
									<?php
									while ($airline = $d_airline->fetch_object()) {
									?>
										<option value="<?= $airline->airline_name ?>" <?= @$_POST['session'] && $_POST['airline'] == $airline->airline_name ? 'selected' : '' ?>><?= $airline->airline_name ?></option>
									<?php
									}
									?>
								</select>
						...
						<input list="sessionList" class="form-control form-control-sm" name="session" value="<?= @$_POST['session'] ?>">
		...
		<div class="row g-0 p-0 m-0">
			<div class=" tabel-area col-sm-12 px-3 pt-3">
				...
				if (isset($_POST['search']) || isset($_GET['session'])) {
					...
					$allAgent = $sesi->get_all_agent();
					$allRa = $sesi->get_all_ra();
									...
									if ($cargo->status == "proced" || $cargo->status == "revisi"): ?>
										<td class="d-flex m-0 py-3 justify-content-between">
											...
											<a href="javascript:;"
												...
												data-bs-toggle="modal" data-bs-target="#editDo"
				...
				<div class="modal-body" style="height: auto;">
					<div class="row gx-2 p-0 m-0">
						<div class="col-md-6">
							...
							<div>
								<select name="noflight" class="form-control form-control-sm modal-select2" id="noflight" data-placeholder="Select Flight" data-dropdown-parent="#editDo" data-control="select2">
									<option></option>
								</select>
							</div>
							...
							<div>
								<select name="agent" class="form-control form-control-sm modal-select2" id="agent" data-placeholder="Select Agent" data-dropdown-parent="#editDo" data-control="select2">
									<option></option>
									<?php
									if (@$allAgent) {
										while ($agent = $allAgent->fetch_object()) {
									?>
						...
						<div class="col-md-6">
							...
							<div class="row g-1 p-0 m-0">
								...
								<div class="col-md-3 d-flex align-items-end justify-content-center">
									<button type="button" class="btn btn-dark btn-sm" data-bs-toggle="modal" data-bs-target="#modalVolume">volume</button>
								</div>
							</div>
							<label for="ragent" class="form-label mt-3">Agent Name</label>
							<div>
								<select name="ragent" class="form-control form-control-sm modal-select2" id="ragent" data-placeholder="Select Regulated Agent" data-dropdown-parent="#editDo" data-control="select2">
									<option></option>
									<?php
									if (@$allRa) {
										while ($ra = $allRa->fetch_object()) {
									?>
											<option value="<?= $ra->ra_id ?>"><?= $ra->ra_name ?></option>
									<?php
										}
									}
									?>
	...
	$(document).ready(function(e) {
		...
		$(".modal-select2").select2({
			...
			dropdownParent: function() {
				let parent = $(this).data('dropdown-parent');
				return $(parent);
			}
		})
